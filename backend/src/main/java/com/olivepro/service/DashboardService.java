...
public class DashboardService {
    ...
    public DashboardResponse getSummary() {
        ...
        double totalVolume = tankRepo.findAll().stream().mapToDouble(com.olivepro.domain.Tank::getCurrentLevel).sum();
        double totalCapacity = tankRepo.findAll().stream().mapToDouble(com.olivepro.domain.Tank::getCapacity).sum();
        ...
        for (int i = 6; i >= 0; i--) {
            ...
            double dayTotal = txRepo.findByCreatedAtBetween(from, to).stream()
                    ...
                    .mapToDouble(com.olivepro.domain.Transaction::getPriceTotal).sum();
            weekly.add(new DashboardResponse.WeeklySalesEntry(from.format(fmt), dayTotal, from.format(fmt)));
        }

        DashboardResponse.AlertsPayloadResponse alerts = new DashboardResponse.AlertsPayloadResponse(0, 0, 0, 0);
        return new DashboardResponse(totalSales, netProfit, totalVolume, totalCapacity, utilization, txCount, weekly, alerts);
    }
}
